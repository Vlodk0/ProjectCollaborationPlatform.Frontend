...
export class AuthService {
  ...
  refreshToken(refreshTokenObj: RefreshToken) {
    //TODO: just as an advice, it's common to name routes as 'refresh-token' rather than 'RefreshToken'
    return this.httpClient.post(
      this.apiUrlSecurity + '/RefreshToken',
      refreshTokenObj
    );
  }
  ...
  createDev(dev: CreateDeveloper) {
    return this.httpClient.post(this.apiUrlDev, dev).subscribe((res: any) => {
      if (res.success) {
        this.router.navigateByUrl('/signin');
      }
    });
